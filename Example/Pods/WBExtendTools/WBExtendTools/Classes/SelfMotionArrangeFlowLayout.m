...
#import "SelfMotionArrangeFlowLayout.h"

@interface SelfMotionArrangeFlowLayout () {
...
@implementation SelfMotionArrangeFlowLayout
...
-(instancetype)init{
    ...
    if (self){
        ...
        self.sectionInset = UIEdgeInsetsMake(5, 12, 5, 12);
        _betweenOfCell = 5.0;
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (NSUInteger index = 0; index < layoutAttributes.count ; index++) {
        ...
        //计算出行数，返回
        float ox = currentAttr.frame.origin.x;
        if ((ox == self.sectionInset.left && self.cellType == AlignWithLeft) ||
            (ox == self.sectionInset.right && self.cellType == AlignWithRight)) {
            sectionNum ++;
        }
    ...
    if (sectionNum > 0 && self.returnSectionNumbersBlock) {
        self.returnSectionNumbersBlock(sectionNum);
    }
